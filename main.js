...
app.post(
  ...
  (req, res) => {
    ...
    console.log(
      `Received file: ${req.file.originalname} (${ext})`
    );
    ...
    const processData = (data) => {
      ...
      return data.map((item) => {
        ...
        for (let key in item) {
          normalizedItem[key.toLowerCase()] =
            item[key];
        }
    ...
    try {
      ...
      if (ext === ".csv") {
        ...
        fs.createReadStream(filePath)
          ...
          .on("end", () => {
            ...
            console.log(
              `Parsed ${processed.length} records from CSV`
            );
      ...
      /* EXCEL */
        ...
        console.log(
          `Parsed ${processed.length} records from Excel`
        );
        ...
        return res
          .status(400)
          .send(
            "Unsupported format. Upload CSV/XLSX"
          );
      }
...
app.post("/start", async (req, res) => {
  ...
  try {
    ...
    const getFinalBody = (item) => {
      ...
      const greeting =
        `Hello ${item.name || "there"},\n\n`;

      return (
        greeting +
        personalize(body, item)
      );
    };
    ...
    if (mode === "single") {
      ...
      await sendMailWithRetry({
        ...
        from:
          `"Growth Studio" <your_verified_email@gmail.com>`,
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        console.log(
          `[${i + 1}/${list.length}] SENT: ${item.email}`
        );
